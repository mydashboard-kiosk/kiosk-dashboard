(function () {
  ...
  // Weather codes -> ASCII icons (no emoji = no mojibake)
  function iconFor(code){
    if (code === 0) return "O";              // clear
    if (code === 1 || code === 2) return "o"; // partly
    if (code === 3) return "C";              // cloudy
    if (code === 45 || code === 48) return "F"; // fog
    if (code >= 51 && code <= 67) return "R";   // rain
    if (code >= 71 && code <= 77) return "S";   // snow
    if (code >= 80 && code <= 82) return "R";   // showers
    if (code >= 95) return "T";                 // thunder
    return "C";
  }
  ...
  function loadEvents(cfg){
    xhrJson("events.json?ts="+Date.now(), function(err, list){
      ...
      if(!upcoming.length){
        // Use a plain hyphen instead of fancy dash
        wrap.innerHTML = '<div class="eventRow"><div class="eventTitle">No upcoming events</div><div class="eventWhen">-</div></div>';
        return;
      }
  ...
  function loadWeather(cfg){
    ...
    xhrJson(url, function(err, data){
      ...
      document.getElementById("wxMain").textContent = textFor(code);
      document.getElementById("wxPlace").textContent = cfg.locationName || "";
      document.getElementById("wxIcon").textContent = iconFor(code);
  ...
  function buildFilmstrip(cfg){
    ...
    xhrJson("photos/photos.json?ts="+Date.now(), function(err, manifest){
      ...
      if(!files.length){
        track.innerHTML = '<div style="color:rgba(255,255,255,0.85);font-weight:900;padding:12px;">Add images to /photos and list them in photos.json</div>';
        return;
      }
      ...
      for(var pass=0; pass<2; pass++){
        for(var i=0;i<files.length;i++){
          html += '<img class="filmImg" src="photos/'+files[i]+'" alt="">';
        }
      }
  ...
  xhrJson("config.json?ts="+Date.now(), function(err, cfg){
    cfg = cfg || { locationName:"Leipsic", lat:41.1092, lon:-83.9957, units:"fahrenheit", forecastDays:10, filmstripSpeedSeconds:35, maxEvents:3 };
